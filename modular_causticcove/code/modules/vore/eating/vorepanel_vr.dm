...
/mob/verb/insidePanel()
	...
	if(!vorePanel)
		if(!isnewplayer(src))
			log_admin("[src] ([type], \ref[src]) didn't have a vorePanel and tried to use the verb.")
		vorePanel = new(src)
...
/datum/vore_look/proc/cached_nom_icon(atom/target)
	...
	if(isobj(target))
		...
		var/mob/M = target
		if(istype(M,/mob/living/simple_animal)) //not generating unique icons for every simplemob(number)
...
/datum/vore_look/ui_static_data(mob/user)
	var/list/data = ..()
...
/datum/vore_look/ui_act(action, params, datum/tgui/ui)
	if(..())
		return TRUE
	...
	switch(action)
		...
		/*if("prey_ability") //This bit requires a specific ability to even trigger it, probably eats and absorbs in one go?
			if(!isliving(ui.user))
				return FALSE
		...
		if("saveprefs")
			if(isnewplayer(host))
				...
				if(choice != "Yes, save.")
					return TRUE
			else if(host.real_name != host.client.prefs.real_name || (!ishuman(host)))
...
/datum/vore_look/proc/pick_from_inside(mob/user, params)
	...
	switch(intent)
		if("Help Out") //Help the inside-mob out
			...
			to_chat(user,span_notice("[span_green("You begin to push [M] to freedom!")]"))
			to_chat(M,span_notice("[host] begins to push you to freedom!"))
			to_chat(OB.owner,span_warning("Someone is trying to escape from inside you!"))
			sleep(50)
			if(prob(33))
				OB.release_specific_contents(M)
				to_chat(user,span_notice("[span_green("You manage to help [M] to safety!")]"))
				to_chat(M, span_notice("[span_green("[host] pushes you free!")]"))
				to_chat(OB.owner,span_alert("[M] forces free of the confines of your body!"))
			else
				to_chat(user,span_alert("[M] slips back down inside despite your efforts."))
				to_chat(M,span_alert("Even with [host]'s help, you slip back inside again."))
				to_chat(OB.owner,span_notice("[span_green("Your body efficiently shoves [M] back where they belong.")]"))
			return TRUE
		...
		if("Devour") //Eat the inside mob
			...
			to_chat(user,span_warning("You begin to [lowertext(TB.vore_verb)] [M] into your [lowertext(TB.name)]!"))
			to_chat(M,span_warning("[host] begins to [lowertext(TB.vore_verb)] you into their [lowertext(TB.name)]!"))
			...
			if((host in OB) && (M in OB)) //Make sure they're still here.
				to_chat(user,span_warning("You manage to [lowertext(TB.vore_verb)] [M] into your [lowertext(TB.name)]!"))
				to_chat(M,span_warning("[host] manages to [lowertext(TB.vore_verb)] you into their [lowertext(TB.name)]!"))
...
/datum/vore_look/proc/pick_from_outside(mob/user, params)
		...
		switch(intent)
			...
			if("move_all")
				...
				for(var/atom/movable/target in host.vore_selected)
					to_chat(target,span_warning("You're squished from [host]'s [lowertext(host.vore_selected)] to their [lowertext(choice.name)]!"))
	...
	switch(intent)
		...
		if("Transfer")
			...
			if(belly_owner != host)
				to_chat(user, span_notice("Transfer offer sent. Await their response."))
				...
				if(accepted != "Yes")
					to_chat(user, span_warning("[belly_owner] refused the transfer!!"))
					return TRUE
				if(!belly_owner || !(belly_owner in range(1, host)))
					return TRUE
				to_chat(target,span_warning("You're squished from [host]'s [lowertext(host.vore_selected.name)] to [belly_owner]'s [lowertext(choice.name)]!"))
				to_chat(belly_owner,span_warning("[target] is squished from [host]'s [lowertext(host.vore_selected.name)] to your [lowertext(choice.name)]!"))
				host.vore_selected.transfer_contents(target, choice)
			else
				to_chat(target,span_warning("You're squished from [host]'s [lowertext(host.vore_selected.name)] to their [lowertext(choice.name)]!"))
				host.vore_selected.transfer_contents(target, choice)
		...
		if("Process")
			...
			if(process_options.len)
				...
				to_chat(user, span_warning("You cannot instantly process [ourtarget]."))
				return FALSE
			...
			if(!istype(b) || b.owner != user)
				to_chat(user, span_warning("[ourtarget] isn't in your belly."))
				return FALSE
			switch(ourchoice)
				if("Digest")
		...
		if("Health Check")
			...
			if(condition)
				to_chat(user, span_warning("\The [target] is currently [condition], they will not be able to [condition_consequences]."))
			return FALSE
