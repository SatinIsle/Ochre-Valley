...
/obj/item/construction/repairkit/structure/attack_obj(obj/O, mob/living/user)
	...
	var/repairskill = 1
	if(user.get_skill_level(/datum/skill/craft/carpentry) >= 1) //we do this incase people have no carpentry skill
		repairskill = user.get_skill_level(/datum/skill/craft/carpentry)
	if(istype(O, /obj/structure/mineral_door/))
		...
		if(doorrepair.obj_integrity < doorrepair.max_integrity)
			...
			if(do_after(user, (150 / repairskill), target = O)) // making this generic carpentry, even though it could be masonry
				qdel(I)
				playsound(user, 'sound/misc/wood_saw.ogg', 100, TRUE)
				doorrepair.icon_state = "[doorrepair.base_state]"
				doorrepair.density = TRUE
				if(!doorrepair.windowed)
					doorrepair.set_opacity(TRUE)
				doorrepair.door_opened = FALSE
				...
				span_notice("[doorrepair.name] is repaired."))
